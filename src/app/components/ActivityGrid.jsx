...
export default function ActivityGrid({ activeCategory }) {
  ...
  const currentData = filteredData.slice(page * itemsPerPage, page * itemsPerPage + itemsPerPage);
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition relative">
        ...
        <div className="px-4 pb-5 mt-3">
          <h3 className="font-semibold text-lg text-gray-900 leading-snug">{item.title}</h3>
          <p className="text-gray-600 text-sm mt-2 line-clamp-3 text-justify">{item.desc}</p>
    ...
    <div className="flex flex-col items-center">
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
        {currentData.map((item) => (
          <EventCard key={item._id || item.id} item={item} />
        ))}
      </div>
        ...
        <div className="flex gap-2 mt-10">
          {Array.from({ length: totalPages }).map((_, i) => (
            <button
              key={i}
              onClick={() => setPage(i)}
              className={`w-3 h-3 rounded-full ${page === i ? "bg-gray-900 scale-110" : "bg-gray-300"}`}
            />
          ))}
        </div>
      )}
      ...
      {selectedEventId && eventDetails[selectedEventId] && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl max-w-4xl w-full mx-4 overflow-hidden relative">
            <button onClick={() => setSelectedEventId(null)} className="absolute top-4 right-4 text-gray-600 hover:text-black">✕</button>
            <div className="grid md:grid-cols-2">
              ...
              <div className="p-6 md:p-8">
                <h2 className="text-2xl font-bold mb-3">{eventDetails[selectedEventId].title}</h2>
                <p className="text-gray-600 mb-6">{eventDetails[selectedEventId].desc}</p>
                ...
                <div className="mt-6 text-sm">
                  ...
                  <a href={eventDetails[selectedEventId].link} target="_blank" className="text-blue-600 hover:underline break-all">
